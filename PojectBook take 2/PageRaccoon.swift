...
class PageRaccoon: UIViewController {

    
    @IBOutlet weak var raccoonLabel: UILabel!
    
    @IBOutlet weak var raccoonImage: UIImageView!
    ...
    let raccoonList = ["honey", "pancakes", "raspberry jam"]
    ...
    override func viewDidLoad() {
        ...
        raccoonLabel.text = raccoonList[0]
        
        
        raccoonImage.image = UIImage(named: raccoonList[0])
        ...
        let raccoonTap = UITapGestureRecognizer(target: self, action: #selector(labelTapped(gestureRecognizer:)))
        raccoonTap.name = "Raccoon"
        raccoonLabel.addGestureRecognizer(raccoonTap)
    ...
    @IBAction func option1Pressed(_ sender: Any) {
        ...
        //pageVC.selectedImage = raccoonImage.image
    ...
    @objc func labelTapped(gestureRecognizer: UITapGestureRecognizer) {
        ...
        if (gestureRecognizer.name == "Raccoon")
        {
            // Handle raccoon tap
            valueList = raccoonList
        }
    ...
    func enableLabelInteractions(){
        ...
        raccoonLabel.isUserInteractionEnabled = true
    }
    func disableLabelInteractions(){
        
        raccoonLabel.isUserInteractionEnabled = false
    }
    ...
    func updateAllPropertiesAndLabels(optionButtonLabel:UIButton!, index: Int){
        ...
        if optionButtonLabel.titleLabel?.text == raccoonList[index]{
            setImagesAndLabels(image: raccoonImage, label: raccoonLabel, propertyList: raccoonList, index: index)
            
            //Making raccoon global:
            // RaccoonVariables.selection = UIImage(named: raccoonList[index])
            
            hideOptionsVew()
            updateOptionsLabels(propertyList: raccoonList)
        }
